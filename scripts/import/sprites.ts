...
async function importSprites() {
  console.log("=== HEIG Odyssey - PokeAPI sprite importer ===");
  ...
  console.log(
    `Found ${raw.species.length} species to process (front, back, front_shiny, back_shiny).`,
  );
  ...
  for (let i = 0; i < raw.species.length; i += CHUNK_SIZE) {
    ...
    process.stdout.write(
      `Processing batch ${Math.floor(i / CHUNK_SIZE) + 1}/${Math.ceil(raw.species.length / CHUNK_SIZE)}...\r`,
    );
    ...
    await Promise.all(
      chunk.map(async (item) => {
        ...
        manifest.sprites[item.id] = {
          ...
          frontShiny: fs.existsSync(frontShinyFile)
            ? frontShinyRel
            : "/pokeball-pixel.svg",
          backShiny: fs.existsSync(backShinyFile)
            ? backShinyRel
            : "/pokeball-pixel.svg",
