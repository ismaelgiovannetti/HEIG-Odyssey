...
const StoredMovesSchema = z
  .array(MoveSchema.extend({ pp: z.number().int().min(0) }))
  .max(4);
...
export function toCollectionEntry(pokemon: UserPokemon): CollectionEntry {
  ...
  const compute = (base: number, key: "atk" | "def" | "spa" | "spd" | "spe") =>
    calculateStat(
      base,
      pokemon.level,
      readStoredStat(pokemon.ivs, key, 15),
      readStoredStat(pokemon.evs, key, 0),
