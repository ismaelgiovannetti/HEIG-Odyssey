...
import { generateTrainingOpponentTeam } from "./opponent-generator";
import type { TrainingDifficulty } from "./difficulty";
import type { Trainer, AIProfile } from "../content/schemas";

export type { TrainingDifficulty } from "./difficulty";

/**
 * Niveau moyen de l'équipe du joueur, borné entre 5 et 100. Une équipe
 ...
 */
export function computeAverageTeamLevel(
  team: Array<{ level: number }>,
): number {
  if (!team || team.length === 0) return TRAINING_MIN_LEVEL;
  return calculateTrainingOpponentLevel(team.map((pokemon) => pokemon.level))
    .opponentLevel;
}

export function difficultyToAIProfile(
 ...
 * contenu complet plutôt que d'un pool figé, au niveau déjà calculé par
 * l'appelant.
 */
export function generateTrainingOpponent(params: {
  ...
  return {
    ...
    defeatCatchline:
      "Simulation terminée. Réessayez pour parfaire votre stratégie.",
