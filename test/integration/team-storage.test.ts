    ...
    pokemon.push(
      await prisma.userPokemon.create({
        data: {
          ...
          moves: [],
          nature: "Hardy",
          teamPosition: i === 0 ? 1 : null,
          boxNumber: i === 0 ? null : 1,
          boxSlot: i === 0 ? null : i,
        },
      }),
    );
...
describe("persistance réelle de l'équipe et du PC", () => {
  it("sauvegarde les transferts et échanges, puis retrouve les mêmes cases après relecture", async () => {
    ...
    const result = await updateActiveTeam(userId, {
      ...
      teamPokemonIds: [b.id],
      pcPlacements: [
        { pokemonId: a.id, boxNumber: 20, boxSlot: 35 },
        { pokemonId: c.id, boxNumber: 1, boxSlot: 1 },
      ],
    });
    ...
    await updateActiveTeam(userId, {
      ...
      teamPokemonIds: [b.id],
      pcPlacements: [
        { pokemonId: a.id, boxNumber: 1, boxSlot: 1 },
        { pokemonId: c.id, boxNumber: 20, boxSlot: 35 },
      ],
    });
    const pcSwapped = await getPlayerCollection(userId);
    expect(pcSwapped.pokemon.find((p) => p.id === a.id)).toMatchObject({
      boxNumber: 1,
      boxSlot: 1,
    });
    expect(pcSwapped.pokemon.find((p) => p.id === c.id)).toMatchObject({
      boxNumber: 20,
      boxSlot: 35,
    });
    ...
    await updateActiveTeam(userId, {
      expectedRevision: 2,
      teamPokemonIds: [b.id, a.id],
    });
    const swapped = await updateActiveTeam(userId, {
      expectedRevision: 3,
      teamPokemonIds: [a.id, b.id],
    });
  ...
  it("refuse le rejeu exact d'une sauvegarde sans déplacer une deuxième fois les créatures", async () => {
    ...
    await expect(updateActiveTeam(userId, input)).rejects.toBeInstanceOf(
      TeamRevisionConflictError,
    );
  ...
  it("ne perd aucune créature si une case est dupliquée ou si une créature étrangère est fournie", async () => {
    ...
    await expect(
      updateActiveTeam(userId, {
        ...
        teamPokemonIds: [a.id],
        pcPlacements: [
          { pokemonId: b.id, boxNumber: 1, boxSlot: 1 },
          { pokemonId: c.id, boxNumber: 1, boxSlot: 1 },
        ],
      }),
    ...
    await expect(
      updateActiveTeam(userId, {
        expectedRevision: 0,
        teamPokemonIds: [other.pokemon[0].id],
      }),
  ...
  it("la base refuse elle-même une double occupation et un emplacement hors limites", async () => {
    ...
    await expect(
      prisma.userPokemon.update({
        where: { id: b.id },
        data: { teamPosition: 1, boxNumber: null, boxSlot: null },
      }),
    ).rejects.toThrow();
    await expect(
      prisma.userPokemon.update({
    ...
    await expect(
      prisma.userPokemon.update({ where: { id: b.id }, data: { boxSlot: 36 } }),
    ).rejects.toThrow();
    expect(await readStoredState(userId)).toEqual(before);
  });
  ...
  it("attribue l'XP au participant de départ même après son transfert dans le PC", async () => {
    const {
      userId,
      pokemon: [participant, replacement],
    } = await createCollection();
    const playerPokemonIds = [participant.id];
    await updateActiveTeam(userId, {
      expectedRevision: 0,
      teamPokemonIds: [replacement.id],
    });
    ...
    const updatedParticipant = await prisma.userPokemon.findUniqueOrThrow({
      where: { id: participant.id },
    });
    const updatedReplacement = await prisma.userPokemon.findUniqueOrThrow({
      where: { id: replacement.id },
    });
    expect(rewards.xpEarned).toBeGreaterThan(0);
    expect(updatedParticipant.teamPosition).toBeNull();
    expect(
      updatedParticipant.level > participant.level ||
        updatedParticipant.experience > participant.experience,
    ).toBe(true);
  ...
  it("annule les gains si la liste des participants contient une créature étrangère", async () => {
    ...
    await expect(
      grantBattleRewards({
        ...
        stageId: "bachelor-1-stage-1",
        winner: "p1",
        playerPokemonIds: [other.pokemon[0].id],
      }),
    ).rejects.toThrow("BATTLE_PARTICIPANTS_UNAVAILABLE");
    expect(await readStoredState(userId)).toEqual(before);
    expect(
      await prisma.battleRecord.findUnique({
        where: { idempotencyKey: battleId },
      }),
    ).toBeNull();
  });
