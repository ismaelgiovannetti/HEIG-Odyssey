...
describe("Battle Vertical Slice (T-US06-06)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("should register a battle session and process a turn to completion with rewards", async () => {
    ...
    // Ce test unitaire couvre le moteur et les services avec PostgreSQL simulé.
    ...
    await expect(
      processBattleTurn(engine.battleId, "other-user", {
        type: "move",
        moveIndex: 0,
      }),
    ...
    const result = await processBattleTurn(engine.battleId, "user-1", {
      type: "move",
      moveIndex: 0,
    });
