...
describe("rangement de la collection", () => {
  ...
  it("accepte le déplacement explicite jusqu'à la dernière case de la dernière boîte", () => {
    const result = buildTeamLayout(owned, {
      ...input,
      pcPlacements: [
        { pokemonId: "p2", boxNumber: 20, boxSlot: 35 },
        { pokemonId: "p3", boxNumber: 1, boxSlot: 1 },
      ],
    });
    expect(result).toContainEqual({
      pokemonId: "p2",
  ...
  it.each(
    [
      ...
      [
        { pokemonId: "p2", boxNumber: 1, boxSlot: 1 },
        { pokemonId: "p3", boxNumber: 1, boxSlot: 1 },
      ], // case partagée
      [
        { pokemonId: "p2", boxNumber: 1, boxSlot: 1 },
        { pokemonId: "p2", boxNumber: 1, boxSlot: 2 },
      ], // créature répétée
      [
        { pokemonId: "p1", boxNumber: 1, boxSlot: 1 },
        { pokemonId: "p3", boxNumber: 1, boxSlot: 2 },
      ], // déjà en équipe
    ].map((pcPlacements) => ({ pcPlacements })),
  )("refuse un rangement incomplet ou dupliqué (%#)", ({ pcPlacements }) => {
    expect(() => buildTeamLayout(owned, { ...input, pcPlacements })).toThrow(
      TeamCompositionInvalidError,
    );
  });
  ...
  it("refuse aussi une créature étrangère placée uniquement dans le PC", () => {
    expect(() =>
      buildTeamLayout(owned, {
        ...input,
        pcPlacements: [{ pokemonId: "foreign", boxNumber: 1, boxSlot: 1 }],
      }),
    ).toThrow(TeamPokemonNotOwnedError);
  });

  it("refuse une équipe vide (T-US05-04)", () => {
    expect(() =>
      buildTeamLayout(owned, { ...input, teamPokemonIds: [] }),
    ).toThrow(TeamCompositionInvalidError);
  });

  it("refuse une équipe de plus de six créatures même toutes possédées (T-US05-04)", () => {
    const sevenOwned = Array.from({ length: 7 }, (_, i) =>
      teamPokemon({ id: `owned-${i}`, boxNumber: null, boxSlot: null }),
    );
    expect(() =>
      buildTeamLayout(sevenOwned, {
        ...input,
        teamPokemonIds: sevenOwned.map((p) => p.id),
      }),
    ).toThrow(TeamCompositionInvalidError);
  });
  ...
  it.each([0, 21, 1.5])("refuse le numéro de boîte %s", (boxNumber) => {
    expect(
      UpdateTeamBodySchema.safeParse({
        ...input,
        pcPlacements: [{ pokemonId: "p2", boxNumber, boxSlot: 1 }],
      }).success,
    ).toBe(false);
  });
  it.each([0, 36, 1.5])("refuse le numéro de case %s", (boxSlot) => {
    expect(
      UpdateTeamBodySchema.safeParse({
        ...input,
        pcPlacements: [{ pokemonId: "p2", boxNumber: 1, boxSlot }],
      }).success,
    ).toBe(false);
  });

  it("gère 700 créatures stockées et refuse un dépôt supplémentaire", () => {
    const pc = Array.from({ length: PC_CAPACITY }, (_, i) =>
      teamPokemon({
        id: `pc-${i}`,
    ...
    expect(
      buildTeamLayout([...team, ...pc], {
        ...input,
        teamPokemonIds: ["p1", "p4"],
      }),
    ).toHaveLength(PC_CAPACITY + 2);
    expect(() => buildTeamLayout([...team, ...pc], input)).toThrow(
      PcCapacityExceededError,
    );
  });
});
