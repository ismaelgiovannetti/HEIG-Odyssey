...
beforeEach(() => {
  ...
  mocks.tx.userProfile.findUnique.mockResolvedValue({
...
describe("service équipe et PC", () => {
  it("lit un instantané privé avec les attaques possédées et les dimensions des boîtes", async () => {
    ...
    expect(mocks.tx.userPokemon.findMany).toHaveBeenCalledWith(
  ...
  it("refuse l'accès avant la fin du recrutement initial", async () => {
    ...
    await expect(getPlayerCollection("user-1")).rejects.toBeInstanceOf(
      TeamOnboardingRequiredError,
    );
    await expect(
      updateActiveTeam("user-1", {
        expectedRevision: 4,
        teamPokemonIds: ["p1"],
      }),
    ).rejects.toBeInstanceOf(TeamOnboardingRequiredError);
  ...
  it("revérifie les entrées même lors d'un appel direct au service", async () => {
    await expect(
      updateActiveTeam("user-1", { expectedRevision: 4, teamPokemonIds: [] }),
    ).rejects.toBeInstanceOf(TeamCompositionInvalidError);
    expect(mocks.transaction).not.toHaveBeenCalled();
  });

  it("refuse une équipe entièrement K.O.", async () => {
    mocks.tx.userPokemon.findMany.mockResolvedValue([
      { ...active, currentHp: 0 },
      stored,
    ]);
    await expect(
  ...
  it("enregistre l'échange puis renvoie la version et le rangement relus", async () => {
    const updated = [
      { ...stored, teamPosition: 1, boxNumber: null, boxSlot: null },
      { ...active, teamPosition: null, boxNumber: 1, boxSlot: 1 },
    ];
    mocks.tx.userPokemon.findMany
      .mockResolvedValueOnce([active, stored])
      .mockResolvedValueOnce(updated);
    const result = await updateActiveTeam("user-1", {
      expectedRevision: 4,
      teamPokemonIds: ["p2"],
    });
    ...
    expect(result.pokemon.find((p) => p.id === "p1")).toMatchObject({
      boxNumber: 1,
      boxSlot: 1,
    });
  });
